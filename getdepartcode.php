...
//checking if there is any groups under selected group or not?
  $StrSql = "SELECT TOP(1) * FROM Departments WHERE GroupID = '1' ORDER BY id DESC";
...
$StrSql = "SELECT TOP(1) * FROM Departments WHERE ID = '".$_GET['GroupID']."' ORDER BY id DESC";
$Result = sqlsrv_query( $connSelComp, $StrSql) or die ( print_r(sqlsrv_errors(), true));
$code = sqlsrv_fetch_array( $Result, SQLSRV_FETCH_ASSOC);
...
$Sql1 = "SELECT TOP(1) * FROM Departments WHERE GroupID = '".$_GET['GroupID']."' ORDER BY id DESC";
$params = array();
